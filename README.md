...
from the tutorial: the Harness Runtime spec, two helper scripts, the exact prompts sent to
each agent, and a script that checks every session from outside the agent.
...
Harness Runtime has a feature for exactly this: **checkpoint-and-fork**. You save the agent's
...
| Path | What it is |
| --- | --- |
| `orders/data.py` | `Order` dataclass and `generate_orders(n)`, a deterministic synthetic order generator |
| `orders/report.py` | The nightly report. Correct, and slow on purpose: every aggregation rescans the full order list |
| `bench.py` | Times `build_report` on 20,000 orders and prints a 16-character checksum of the output |
| `tests/` | 15 pytest tests plus a golden report for 500 orders. They pin the report's shape and values |
| `specs/agents.yaml` | The Harness Runtime spec: Claude Code on a `mv-2vcpu-4gb` sandbox with DigitalOcean-hosted inference |
| `specs/start-session.py` | Creates the session by posting the spec to the Managed Agents API (see [specs/README.md](specs/README.md) for why) |
...
- `pytest -q` runs 15 tests, including a golden-file comparison for 500 orders.
- `python bench.py` prints a checksum of the full 20,000-order report. On `main` it is
...
Where the time goes: `repeat_customer_share` alone is about 76% of the runtime because it
...
You need a DigitalOcean team with Managed Agents enabled, a personal access token exported
as `DIGITALOCEAN_ACCESS_TOKEN`, and `doctl` 1.168.0-beta or later (the `harness-runtime`
commands are in the beta builds on the [doctl releases page](https://github.com/digitalocean/doctl/releases)).
No Anthropic key is needed; the spec uses DigitalOcean-hosted inference.
...
itself. Confirm from outside the agent:
